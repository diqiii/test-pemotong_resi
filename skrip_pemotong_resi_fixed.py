...
with st.sidebar:
    ...
    tebal_garis_ui = st.slider("Tebal Garis Minimal (px)", 5, 60, 10,
        help="Naikkan angka ini jika resi kepotong di tengah.")

    st.markdown("---")
    st.markdown("**🔬 MODE OCR**")
    ocr_agresif = st.checkbox("Mode Agresif (lebih lambat, lebih akurat)", value=True)
...
def ocr_dari_gray(crop_gray, agresif=True):
    ...
    thresh_adaptive = cv2.adaptiveThreshold(img_2x, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 31, 2)
    ...
    if agresif:
        ...
        thresh_denoised = cv2.adaptiveThreshold(denoised, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 21, 10)
...
def proses_tiktok(img_asli, database_nomor):
    ...
    if y_trim_bawah <= y_trim_atas:
        y_trim_bawah = h_asli; y_trim_atas = 0
    ...
    garis_ditemukan = []
    in_garis = False; start_y = 0
    for y in range(h):
        if is_garis[y]:
            if not in_garis: in_garis = True; start_y = y
        else:
            if in_garis:
                if (y - start_y) >= tebal_garis_ui: garis_ditemukan.append((start_y, y))
                in_garis = False
    ...
    for i in range(total_segmen):
        y_atas = batas_y[i]; y_bawah = batas_y[i+1]
        tinggi_resi = y_bawah - y_atas
        if tinggi_resi < 270: continue
        ...
        if is_resi_terakhir:
            area_ocr_bgr[int(ocr_h*0.65):, int(w*0.72):] = 255

        teks = ocr_dari_gray(cv2.cvtColor(area_ocr_bgr, cv2.COLOR_BGR2GRAY), agresif=ocr_agresif)
        teks_upper = teks.upper()
        if "BATAL" in teks_upper or "CANCELED" in teks_upper or "CANCELLED" in teks_upper: continue

        nomor = ekstrak_nomor_tiktok(teks)
        if not nomor:
            full_bgr = crop_untuk_disimpan.copy()
            if is_resi_terakhir: full_bgr[int(tinggi_resi*0.65):, int(w*0.72):] = 255
            teks_full  = ocr_dari_gray(cv2.cvtColor(full_bgr, cv2.COLOR_BGR2GRAY), agresif=ocr_agresif)
...
def ekstrak_tanggal_shopee(teks):
    ...
    if m:
        try: return datetime(int(m.group(1)), int(m.group(2)), int(m.group(3)), int(m.group(4)), int(m.group(5)))
        except ValueError: pass
    ...
    if m:
        ...
        try: return datetime(int(m.group(3)), bulan_map[m.group(2).lower()], int(m.group(1)))
        except ValueError: pass
...
def proses_shopee(img, database_nomor):
    ...
    is_gap       = (row_std < 5) & (row_mean < putih_kertas - 2)

    gaps = []; in_gap = False; start_y = 0
    for y in range(h):
        if is_gap[y]:
            if not in_gap: in_gap = True; start_y = y
        else:
            if in_gap:
                if y - start_y >= 10: gaps.append((start_y, y))
                in_gap = False
    if not gaps: return hasil_sementara
    ...
    for i in range(len(gaps)):
        ...
        if nomor_pesanan in database_nomor: continue
        ...
        for j in range(len(d['text'])):
            ...
            if any(k in word for k in ["ID","ORDER","TIME","WAKTU","PAYMENT","PEMBAYARAN",nomor_pesanan]):
                ...
                if y_bottom > lowest_text_y: lowest_text_y = y_bottom

        cut_y = int(lowest_text_y) + 30
        c_std = np.std(card_gray[:, int(w*0.2):int(w*0.8)], axis=1)
        for y_scan in range(int(lowest_text_y)+5, min(int(lowest_text_y)+155, y_end_card-y_start_card)):
            if c_std[y_scan] < 8: cut_y = y_scan + 2; break
...
# ==========================================
# FUNGSI PROSES & TAMPIL
# ==========================================
def jalankan_proses(uploaded_files, platform):
    temp_dir = "temp_hasil"
    ...
    if platform == "TikTok Shop":
        ...
        for idx, file in enumerate(uploaded_files):
            progress_bar.progress(int(idx/len(uploaded_files)*100),
                text=f"Memproses file {idx+1}/{len(uploaded_files)}...")
        ...
        for c, (nomor, crop) in enumerate(semua_hasil, 1):
            cv2.imwrite(os.path.join(temp_dir, f"TikTok_{c:03d}_{nomor}.jpg"), crop)
        ...
        semua_hasil.sort(key=lambda x: x[0], reverse=True)
        for c, (tanggal, nomor, crop) in enumerate(semua_hasil, 1):
            tgl_str = tanggal.strftime("%Y%m%d") if tanggal.year > 1970 else "tglUnknown"
            cv2.imwrite(os.path.join(temp_dir, f"Shopee_{c:03d}_{tgl_str}_{nomor}.jpg"), crop)

    progress_bar.progress(100, text="Selesai! ✅")
    return sorted(os.listdir(temp_dir)), temp_dir


def tampilkan_hasil(hasil_files, temp_dir):
    if not hasil_files:
        st.error("Tidak ada resi valid. Coba aktifkan Mode Agresif atau sesuaikan slider.")
        return
    ...
    for filename in hasil_files:
        with open(os.path.join(temp_dir, filename), "rb") as f:
            ...
            js_arr.append(f'{{name:"{filename}",data:"data:image/jpeg;base64,{b64}"}}')

    st.components.v1.html(f"""
    <button id="dl" style="width:100%;padding:15px;background:#FF4B4B;color:white;
...
# ------------------------------------------------------------------
# SOLUSI TIMEOUT ANDROID:
#
# Root cause: Android Chrome memutus WebSocket saat file picker
# terbuka > 3-4 detik karena tab dianggap tidak aktif.
#
# Solusi: Upload SATU PER SATU dalam batch kecil.
# Dengan mode "Satu per satu", setiap kali user pilih 1 foto,
# langsung dikirim ke server sebelum WebSocket sempat timeout.
# Semua hasil dikumpulkan di st.session_state dan diproses bersama
# saat tombol "Proses Semua" ditekan.
#
# Ini cara yang paling sederhana dan 100% berhasil di Community Cloud.
# ------------------------------------------------------------------

# Inisialisasi session state untuk tampung foto satu per satu
if "kumpulan_file" not in st.session_state:
    st.session_state.kumpulan_file = []   # list of (nama, bytes)
if "sudah_proses" not in st.session_state:
    st.session_state.sudah_proses = False

mode = st.radio(
    "Mode Upload:",
    ["📦 Sekaligus (desktop/WiFi cepat)", "📱 Satu per satu (HP Android — anti timeout)"],
    help="Pilih 'Satu per satu' jika foto sering gagal terpilih di HP."
)

if mode == "📦 Sekaligus (desktop/WiFi cepat)":
    uploaded_files = st.file_uploader(
        "Pilih semua foto resi sekaligus",
        type=["jpg","jpeg","png","webp"],
        accept_multiple_files=True,
        label_visibility="collapsed"
    )
    if uploaded_files:
        st.info(f"✅ {len(uploaded_files)} foto siap.")
    if st.button("🚀 Proses Resi", use_container_width=True, type="primary"):
        if not uploaded_files:
            st.warning("Pilih foto dulu!")
        else:
            with st.spinner("Membedah resi..."):
                hasil_files, temp_dir = jalankan_proses(uploaded_files, platform)
            tampilkan_hasil(hasil_files, temp_dir)

else:
    # MODE SATU PER SATU
    # Setiap upload langsung disimpan ke session_state
    # WebSocket hanya aktif sebentar per 1 foto → tidak sempat timeout
    st.markdown(f"**Foto terkumpul: {len(st.session_state.kumpulan_file)} foto**")

    file_baru = st.file_uploader(
        "Pilih 1 foto (ulangi untuk foto berikutnya)",
        type=["jpg","jpeg","png","webp"],
        accept_multiple_files=False,   # ← kunci: SATU file per upload
        label_visibility="collapsed",
        key=f"uploader_{len(st.session_state.kumpulan_file)}"
        # key berubah setiap kali ada file baru → widget reset otomatis
    )

    col_tambah, col_hapus = st.columns(2)

    with col_tambah:
        if st.button("➕ Tambah Foto Ini", use_container_width=True, disabled=(file_baru is None)):
            if file_baru:
                nama  = file_baru.name
                bytes_data = file_baru.read()
                # Cek duplikat nama
                nama_existing = [n for n, _ in st.session_state.kumpulan_file]
                if nama not in nama_existing:
                    st.session_state.kumpulan_file.append((nama, bytes_data))
                    st.session_state.sudah_proses = False
                    st.rerun()
                else:
                    st.warning(f"'{nama}' sudah ada di daftar.")

    with col_hapus:
        if st.button("🗑️ Hapus Semua", use_container_width=True,
                     disabled=(len(st.session_state.kumpulan_file) == 0)):
            st.session_state.kumpulan_file = []
            st.session_state.sudah_proses  = False
            st.rerun()

    # Tampilkan daftar foto yang sudah dikumpulkan
    if st.session_state.kumpulan_file:
        with st.expander(f"📋 Daftar {len(st.session_state.kumpulan_file)} foto terkumpul"):
            for i, (nama, _) in enumerate(st.session_state.kumpulan_file):
                col_n, col_del = st.columns([4, 1])
                with col_n:
                    st.text(f"{i+1}. {nama}")
                with col_del:
                    if st.button("✕", key=f"del_{i}"):
                        st.session_state.kumpulan_file.pop(i)
                        st.rerun()

    # Tombol proses semua foto yang sudah terkumpul
    if st.button("🚀 Proses Semua Foto", use_container_width=True, type="primary",
                 disabled=(len(st.session_state.kumpulan_file) == 0)):

        # Bungkus bytes ke objek file-like agar kompatibel dengan jalankan_proses
        class FakeFile:
        ...
        fake_files = [FakeFile(n, b) for n, b in st.session_state.kumpulan_file]

        with st.spinner("Membedah resi..."):
            hasil_files, temp_dir = jalankan_proses(fake_files, platform)

        st.session_state.sudah_proses = True
        tampilkan_hasil(hasil_files, temp_dir)
